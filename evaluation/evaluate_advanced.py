...
def coerce_coords(obj):
    ...
    if a.dtype == object:
        try:
            a = np.stack([np.asarray(x, dtype=float).reshape(3) for x in a], axis=0)
            return a
        except Exception:
            pass
...
def evaluate_with_scaling(df, scale_factor, dataset="Geom", n_samples=1000, debug=False):
    ...
    for ridx, row in df_sub.iterrows():
        try:
            ...
            if coords.shape[0] != types.shape[0] or coords.shape[1] != 3:
                raise ValueError(f"length mismatch types={types.shape} coords={coords.shape}")
            if types.size == 0:
                raise ValueError("empty after filtering invalid Z")
                
            mols.append(row_to_placeholder_mol(types, coords, scale=scale_factor, center=False))
    ...
    try:
        m3d, rd_mols = get_3D_edm_metric(mols, train_mols=None, dataset_name=dataset, use_mmff=False)
...
def main():
    ap = argparse.ArgumentParser(description="Fixed evaluation for Mol-MMaDA generated molecules")
    ap.add_argument("--parquet", required=True, help="Path to generated_mols.parquet")
    ap.add_argument("--engine", default="pyarrow", choices=["pyarrow", "fastparquet"])
    ap.add_argument("--n", type=int, default=1000, help="Sample size for evaluation")
    ap.add_argument("--dataset", default="Geom", choices=["QM9", "Geom"], help="Dataset rules for bond checking")
    ap.add_argument("--out", default="eval_results_fixed.json", help="Output JSON file")
    ap.add_argument("--analyze-only", action="store_true", help="Only analyze coordinate ranges")
    ap.add_argument("--test-scales", action="store_true", help="Test multiple scaling factors")
    ap.add_argument("--scale", type=float, default=0.1, help="Single scaling factor to test")
    ...
    if args.test_scales:
        ...
        for scale in scales_to_test:
            print(f"\n{'='*50}")
            results = evaluate_with_scaling(df, scale, args.dataset, args.n, args.debug)
            if results:
                ...
                if score > best_score:
                    ...
                    print(f"*** NEW BEST SCORE: {score:.4f} with scale={scale} ***")
        
        if best_results:
            print(f"\n{'='*50}")
            print(f"BEST RESULTS (scale={best_results['scale']}):")
            print(f"3D mol_stable: {best_results['metrics_3d'].get('mol_stable', 0):.4f}")
            print(f"3D atom_stable: {best_results['metrics_3d'].get('atom_stable', 0):.4f}")
        ...
        results = evaluate_with_scaling(df, args.scale, args.dataset, args.n, args.debug)
        if results:
            with open(args.out, "w") as f:
